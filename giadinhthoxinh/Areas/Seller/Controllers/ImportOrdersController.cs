...
namespace giadinhthoxinh.Areas.Seller.Controllers
{
    public class ImportOrdersController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "PK_iImportOrderID,FK_iAccountID,dtDateAdded,sDeliver,iState")] tblImportOrder tblImportOrder)
        {
            var userId = (int)Session["idUser"];
            ...
            {
                tblImportOrder.FK_iSupplierID = iSupplierID;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "PK_iImportOrderID,FK_iAccountID,dtDateAdded,sDeliver,iState")] tblImportOrder tblImportOrder)
        {
            var userId = (int)Session["idUser"];
            var iSupplierID = db.tblSuppliers.Where(x => x.FK_iAccountID == userId).Select(x => x.PK_iSupplierID).FirstOrDefault();
