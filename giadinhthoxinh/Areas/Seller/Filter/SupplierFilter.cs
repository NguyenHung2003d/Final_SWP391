...
namespace giadinhthoxinh.Areas.Seller.Filter
{
    public class SupplierFilter : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if (supplier != null&&supplier.iState == 0)
            {
                // Lưu thông báo vào TempData
                filterContext.Controller.TempData["Message"] = "Bạn phải đợi xác thực xong.";
                // Chuyển hướng đến trang index của user
                filterContext.Result = new RedirectToRouteResult(
                    new RouteValueDictionary(new { controller = "Home", action = "Index", area = "Seller" }));
            }
