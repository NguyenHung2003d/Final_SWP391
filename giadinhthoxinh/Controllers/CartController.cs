...
namespace giadinhthoxinh.Controllers {
    public class CartController : Controller {
        ...
        public ActionResult TrackOrder(int orderId) {
            var order = db.tblOrders.Include(o => o.tblCheckoutDetails).FirstOrDefault(o => o.PK_iOrderID == orderId);
            if (order == null) {
                return HttpNotFound();
            }
        ...
        public ActionResult ThongTinHoaDon() {
            var userId = (int)Session["idUser"];
            var orders = db.tblOrders.Where(x => x.FK_iAccountID == userId).Select(x => x.PK_iOrderID).ToList();
            List<tblCheckoutDetail> checkoutDetails = new List<tblCheckoutDetail>();

            foreach (var order in orders) {
                var details = db.tblCheckoutDetails.Where(x => x.FK_iOrderID == order).ToList();
                checkoutDetails.AddRange(details);
            }

            var totalAmount = checkoutDetails.Sum(item => item.fPrice * item.iQuantity);
        ...
        private string CreateVNPayPaymentRequest(decimal totalAmount) {
            ...
            vnpay.AddRequestData("vnp_TmnCode", "YLWQMKV2");
            ...
            string paymentUrl = vnpay.CreateRequestUrl("https://sandbox.vnpayment.vn/paymentv2/vpcpay.html", "CU9EKGPEBOGY8UU2HX0S8C4IYZ5WN2WS");
            return paymentUrl;
        }
        ...
        public ActionResult VNPayReturn() {
            ...
            bool checkSignature = vnpay.ValidateSignature(vnp_SecureHash, "CU9EKGPEBOGY8UU2HX0S8C4IYZ5WN2WS");
